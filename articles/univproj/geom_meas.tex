...
\vspace{1ex}
\textit{Upper and lower derivatives lemmas: For Rado measures and
...
\vspace{1ex}
\textbf{Proof:}
We may assume that radon measures are finite, otherwise we can always take
...
Then $\inf\{r\;|\;B(a,r)\in\mathcal F\}=0$ for each $a\in A$ and hence
\emph{Filling Theorem} provides us with a countable collection $\mathcal G$ of disjoint
balls in $\mathcal F$ such that
...
\vspace{1ex}
\textbf{Proposition:} \textit{Let $\mu$ be a Radon measure and $\nu$ be a Radon
or Vector measure. Then
\begin{itemize}
...
\vspace{1ex}
\textbf{Radon-Nikodym Theorem:} \textit{Let $\nu$ be a vector measure and $\mu$ a Radon
measure on $\mathbb{R}^n$. Then
\[
    \nu(A) = \int_AD_\mu\nu\,d\mu
\]}
...
\vspace{2ex}
This gives us a representation of a vector valued measure $\mu$ as $\mu=
D_{|\mu|}\mu\,|\mu|$. And we can define the integration with respect to such
measure as
\[
    \int fd\mu = \int f\cdot D_{|\mu|}\mu\,d|\mu|
\]
...
\vspace{2ex}
\textbf{Proposition:} \textit{Alternatively we can define an $n$-rectifiable set by
