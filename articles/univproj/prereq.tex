...
A projection on the tangent space is associated to $T_xM\times (T_xM)^\bot$.
...
matrix multiplication notion to shoten equations, even when dealing with things
...
as you will see in Section 7, by using subtitutions. Although, I switch later
...
profound statements, I've avoided them here to maintian the holistic picture I aim 
to present and not to have just another copy of an already existing book. This
necessarily means sucreficing some depth, as I have not included few necessery 
proofs and results.
...
generalisations, are primarly sourced from "Measure theory and fine properties
of functions".
...
There are also more substential covering results, crucial for extension 
...
inovolving the extensive rewrite of inequalities and introduction of different
constants. I initially inteded toto revise them for greater clearity, but after
several failed attempts, when I had to modify constants and propogate the change
...
\vspace{1ex}
\textbf{Remark:} I concidered extending the theorem to vector-valued
...
\vspace{2ex}
Beyond representating of measure (or continuous linear forms), we can view this
process from another angle. If we associate to a linear continious
form to a shape, then function $f$ describes certain geometric propertise of
